...
def index():
    ...
    stats = {
        ...
        "freelancers": conn.execute("SELECT COUNT(*) FROM users WHERE role='freelancer'").fetchone()[0],
...
def dashboard():
    ...
    if role == "client":
        ...
        data["open_jobs"] = conn.execute("SELECT COUNT(*) FROM jobs WHERE status='open'").fetchone()[0]
...
def profile(user_id):
    ...
    avg_rating = conn.execute("SELECT AVG(rating) FROM reviews WHERE reviewee_id=?", (user_id,)).fetchone()[0] or 0
    reviews_count = conn.execute("SELECT COUNT(*) FROM reviews WHERE reviewee_id=?", (user_id,)).fetchone()[0]
    if user["role"] == "freelancer":
        ...
        completed_count = conn.execute(
            "SELECT COUNT(*) FROM proposals WHERE freelancer_id = ? AND status = 'accepted'",
            (user_id,),
        ).fetchone()[0]
